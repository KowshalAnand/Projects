...
select first_name, last_name,job_title
 ...
 # 3. Using HR Schema, Write a Query to fetch all the departments and 
 # the names of the person managing them
 
 select d.department_id,department_name,d.manager_id,first_name,last_name
	from departments d left join employees e
    on d.manager_id=e.employee_id;
    
 

# 4. Using HR Schema, Write a Query to fetch all the departments and 
# the names of the person managing them in America Region.

select d.department_id,department_name,d.manager_id,first_name,last_name,region_name
	from departments d left join employees e
    on d.manager_id=e.employee_id
...
# 6. Using HR Schema, Write a query to fetch the 
# names of the employee who is not assigned to any departement yet.
...
#7. Using HR Schema, Write a query to identify the list of names of employees who are not assigned any work 
# and also the list of departments that are unstaffed so that the employees can be utilised to staff the departments.
...
#8	Using HR Schema, Write a query to list the names of all people who report to the same person.
 
 select e1.first_name,e1.last_name,e2.first_name,e2.last_name,e1.manager_id
	from employees e1 join employees e2
    on e1.manager_id=e2.manager_id
    and e1.employee_id <> e2.employee_id;


 #9	Using HR Schema, Write a query to list the names of all people who report to the same person in department Accounting (i.e.110).
	
 select e1.first_name,e1.last_name,e2.first_name,e2.last_name,e1.manager_id,e1.department_id
	from employees e1 join employees e2
    on e1.manager_id=e2.manager_id
    and e1.employee_id <> e2.employee_id
    where e1.department_id='110';
 
 # 10 Write a query in SQL to display the first name of all employees 
 # and the first name of their manager including those who does not working under any manager.
	
select e1.first_name as emp_name,e2.first_name as mang_name,e1.manager_id
	from employees e1 join employees e2
    using(employee_id);
...
# 11 Using the HR Schema, Write a Query to print the name and  a message 'Well Paid' or 'Underpaid' if the salary is above or below 6500$ respectively.
	
    select first_name,salary,
		case when salary>=6500 then 'Well Paid'
			when salary <6500 then 'Under Paid' end as Salary_Review
		from employees;
	

# 12. Write a query in SQL to display the name of the department, average salary and number of employees working in that department who got commission.
	
		select department_name,avg(salary),count(employee_id) as Employee_count,commission_pct
