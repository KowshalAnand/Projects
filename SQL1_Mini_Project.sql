...
-- 1.	Show the percentage of wins of each bidder in the order of highest to lowest percentage.
	...
	SELECT bd.BIDDER_ID,bd.BIDDER_NAME,
		round((((count(case when bs.BID_STATUS='won' then 1 end)) /(count(case when bs.BID_STATUS!='Cancelled' then 1  end)))*100),2) as Win_Percentage
		FROM ipl_bidder_details bd
		INNER JOIN ipl_bidding_details bs
		ON bd.BIDDER_ID=bs.BIDDER_ID
...
-- 2.	Display the number of matches conducted at each stadium with stadium name, city from the database.
	...
	select s.Stadium_name,s.City,count(s.stadium_id) as Number_of_matches
		from ipl_stadium s left join ipl_match_schedule m
...
/* Updating the Match_Winner details based on the Win_Details*/
...
commit;
	...
	SELECT st.STADIUM_NAME AS Stadium_Name,
round((((SUM(CASE
			WHEN TOSS_WINNER=MATCH_WINNER THEN 1
			ELSE NULL
			END))/(COUNT(ms.SCHEDULE_ID)))*100),2) AS Win_Percent
FROM ipl_stadium st 
...
-- 5.	Show the team id who won the match as per the win details.
	...
	select Win_details,
		case When match_winner=1 then team_id1
			when match_winner=2 then team_id2 end as Team_id
		from ipl_match;

-- 6.	Display total matches played, total matches won and total matches lost by team along with its team name.

		select ts.Team_id,t.Team_name,sum(matches_won+matches_lost) as Total_match,
...
-- 7.	Display the bowlers for Mumbai Indians team.
	...
	select player_id,Team_name,player_name
		from  ipl_player join ipl_team_players
        using(player_id) join ipl_team
        using (team_id)
...
select Team_name,count(player_id) as All_Rounder
		from  ipl_player join ipl_team_players
        using(player_id) join ipl_team
        using(team_id)
